...
-(instancetype)initWithParams:(KKCaptureParams*) params error: (NSError*__autoreleasing _Nullable * _Nullable)error;
{
    if (self = [super init])
    {
        ...
        // 获取当前方向的摄像头
        NSArray *captureDeviceArray = [cameras filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"position == %d", _params.devicePosition]];
        ...
        // 设置分辨率
        if ([self.captureSession canSetSessionPreset:self.params.sessionPreset])
        {
            self.captureSession.sessionPreset = self.params.sessionPreset;
        }
        ...
        if (self.params.devicePosition == AVCaptureDevicePositionFront && self.captureConnection.supportsVideoMirroring)
        {
            self.captureConnection.videoMirrored = YES;
        }
        self.captureConnection.videoOrientation = self.params.videoOrientation;

        self.previewLayer = [AVCaptureVideoPreviewLayer layerWithSession:self.captureSession];
        self.previewLayer.connection.videoOrientation = self.params.videoOrientation;
        ...
        // 设置帧率
        [self adjustFrameRate:self.params.frameRate];
    }
    return self;
}
...
- (NSError *)adjustFrameRate:(NSInteger)frameRate
{
    ...
    self.captureDeviceInput.device.activeVideoMinFrameDuration = CMTimeMake(1, (int)self.params.frameRate);
    self.captureDeviceInput.device.activeVideoMaxFrameDuration = CMTimeMake(1, (int)self.params.frameRate);
...
{
    self.params.sessionPreset = sessionPreset;
    if ([self.captureSession canSetSessionPreset:self.params.sessionPreset])
    {
        self.captureSession.sessionPreset = self.params.sessionPreset;
    }
